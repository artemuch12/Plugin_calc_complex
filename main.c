...
/*В качестве входных данных для библиотек принимается выражения вида 
 * ./lib/lin<namelibrary>.so. Предполагается, что будут подключены биб-
 * -лиотеки содержащие внутри себя по одной функции, при этом названия 
 * функций будет совпадать с именнем библиотек. В добавок ко всему этому 
 * функции дожны содержать по 3 входных параметра: первый и второй аргу-
 * -мент отвечают за переменные скоторыми будет произведены манипуляции
 * третий аргумент есть результат манипуляции над ранними аргументами.*/
